...
import UserReportModal from '@/features/user/report/ui/UserReportModal';
import AdminDeleteModal from '@/entities/admin/ui/AdminDeleteModal';
import useQnA from '@/entities/board/qna/apis/useQnA';
import { useParams, useRouter } from 'next/navigation';
import SendIcon from '@mui/icons-material/Send';
import Comment from '@/features/qna-reply/ui/Comment';
      ...
      <div className="flex-col justify-center items-center relative">
        <div className="w-[1200] h-[550] mt-8 bg-white flex-col justify-center items-center rounded-xl relative">
          <div className="flex-col pt-10 justify-center items-center mx-[50]">
            <div className="w-full flex justify-between">
              <div className="flex gap-5 justify-center items-center">
                <div className="min-w-[70] w-auto p-2 h-[30] bg-[#0095E8] flex justify-center items-center text-white rounded-full">
                  {qnaDetail?.language}
                </div>
              ...
              <div className="flex items-center !text-xl">
                {nickname === 'admin' ? (
                  <button
                    onClick={() => setIsAdminDelete(!isAdminDelete)}
                    className="underline text-[#656565] mr-5"
                  >
                    삭제
                  </button>
                ) : (
            ...
            <div className="flex gap-4 mt-2">
              <div>
                {qnaDetail?.userId} | {qnaDetail?.createDate}
              </div>
          ...
          <div className="flex w-[1100] justify-between mx-[50] absolute bottom-5">
            <button
              onClick={() => {
                toggleIsComment();
                fetchComment(String(params.qna_Id));
              }}
            ...
            <div className="flex">
              <div className="w-[75px] h-[45px] shadow-xl bg-[#0095E8] mr-5 text-white flex justify-center items-center rounded-2xl border text-sm border-[#D0D0D0]">
                답변 작성
              </div>
              <Link href="/qna">
                <div className="w-[75px] h-[45px] bg-[#444444] border text-white flex justify-center items-center rounded-2xl border-1 border-[#D0D0D0] shadow-xl">
                  목록
                </div>
              </Link>
            </div>
        ...
        {isComment === true && isReply === false && (
          <div className="w-[1200] h-auto mb-10 flex-col bg-white rounded-md mt-12">
            <div className="flex-col justify-between mb-7 mx-[50] pt-9">
              <div>
                댓글
                <span className="text-[#0095E8] ml-1">
                  {commentList?.length}
                </span>
              </div>
              <textarea
                ...
                className="resize-none w-full border border-[#D0D0D0] bg-[#EBEBEB] rounded-lg h-24 flex justify-start items-start"
                placeholder="작성할 댓글의 내용을 입력해주세요."
              />
              <button
                className="flex justify-end w-full"
                onClick={(e) => createComment(String(params.qna_Id))}
              >
                <SendIcon />
              </button>
            </div>
            {commentList ? (
              <>
                {fetchCommentList?.map((comment: QnaCommentType) => (
                  <Comment key={comment.replyId} comment={comment} />
                ))}
        ...
        {isReport === true ? (
          <div className="w-full h-full overflow-y-hidden flex justify-center items-center fixed  z-10 inset-0 bg-gray-500/50">
            <UserReportModal setIsReport={setIsReport} />
          </div>
        ) : (
          <></>
        )}
        {isAdminDelete === true ? (
          <div className="w-full h-full overflow-y-hidden flex justify-center items-center fixed  z-10 inset-0 bg-gray-500/50">
            <AdminDeleteModal setIsAdminDelete={setIsAdminDelete} />
          </div>
        ) : (
          <></>
        )}
