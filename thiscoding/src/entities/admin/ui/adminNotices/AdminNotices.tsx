...
import { AdminNoticesListDTO } from '@/entities/admin/types/AdminNoticesListDTO';
import { useRouter } from 'next/navigation';
import { axiosInstance } from '@/shared/apis/axiosConfig';
import { axiosWithAuth } from '@/shared/apis/axiosWithAuth';
const AdminNotices = () => {
    ...
    <>
      <>
        <div className="w-[950] h-[650] bg-[#FFFFFF] ml-24 rounded-2xl border-[#EBEBEB] relative">
          <div className="absolute flex justify-end mt-3 right-[50] w-full font-thin">
            개별 문의 : abc123@naver.com
          </div>
              ...
              <>
                <tbody className="cursor-pointer">
                  {NoticesList?.map((notices) => (
                    <AdminNoticesContent key={notices.id} notices={notices} />
                  ))}
                </tbody>
              </>
            ) : (
          ...
          <Link
            href="/admin/notices/post-notices"
            className="w-[90] h-[40] border rounded-2xl flex justify-center items-center border-black absolute right-[50] bottom-[30] text-[#0095E8]"
          >
            글쓰기
          </Link>
        </div>
