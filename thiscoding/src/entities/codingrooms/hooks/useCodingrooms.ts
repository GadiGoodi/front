...
import WebSocketService from '@/entities/codingrooms/apis/codingroomsWebSocket';
import {
  CodeGetData,
  CodingroomsGetData,
  codeSnippets,
  languageVersions,
  languageExtensions,
} from '@/entities/codingrooms/types/CodingroomsDTO';
...
const useCodingrooms = () => {
  ...
  const [fileTabs, setFileTabs] = useState([
    {
      fileName: 'main',
      value: codingrooms?.value as string,
      codeId: codingrooms?.codeId as string,
    },
  ]);
  ...
  // 이름을 변경할 파일 탭 요소 번호
  const [editingFileTabIndex, setEditingFileTabIndex] = useState<number | null>(
    null,
  );
  ...
  const handleEditorChange: OnChange = (value) => {
    ...
    setFileTabs(
      fileTabs.map((f) =>
        f.fileName === activeFileTab.fileName
          ? { ...f, value: value || '' }
          : f,
      ),
    );
    ...
    if (WebSocketService.subscriptions[`tab-${activeFileTab.codeId}`]) {
      WebSocketService.send(
        '/pub/broadcast-code',
        JSON.stringify({ roomId: codingrooms?.roomId, value: value as string }),
      );
    }
  };
  ...
  const handleAddFileTab = () => {
    ...
    const newFileTab = {
      fileName: newFileTabName,
      value: codeSnippets[codingrooms?.language as string],
      codeId: '',
    };
  ...
  const handleRemoveFile = (
    fileTab: { fileName: string; value: string; codeId: string },
    index: number,
  ) => {
    ...
    if (activeFileTab.fileName === fileTab.fileName) {
      ...
      // 해당 인덱스 탭으로 새로이 set
      setActiveFileTab(
        updatedFileTabs[newIndex] || { name: '', value: '', codeId: '' },
      );
      setIsTabChanged(!isTabChanged);
    }
  };
  ...
  const changeFileTabName = async (
    ...
    if (e.key === 'Enter') {
      ...
      if (
        fileTabs.some(
          (f) => f.fileName === `${tmpFileTabName}` && f.codeId != null,
        )
      ) {
        alert('파일명이 중복됩니다.');
        closeFileTabInput();
