...
const useSignUp = () => {
  ...
  const handleSendAuthCode = async (email: string) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/api/auth/join-code?email=${email}`,
      );
      setIsCodeSent(true);
  ...
  const handleVerifyAuthCode = async (email: string, code: string) => {
    ...
    await axios
      .post('http://localhost:8080/api/auth/join-code/check', {
        email,
        code,
      })
      .then((res) => {
  ...
  const checkEmailAvailability = async (email: string) => {
    ...
    axios
      .get(`http://localhost:8080/api/auth/email?email=${email}`)
      .then((res) => {
        if (res.data === false) {
  ...
  const register = async (signupInfo: SignUp) => {
